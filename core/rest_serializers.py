...
class IssueSerializer(serializers.BaseSerializer):
  ...
  def to_representation(self, instance):
    return {
      ...
      'pages': [{
        'sequence': p.sequence,
        'url': settings.BASE_URL + reverse('api_page', args=[p.issue.title.lccn, p.issue.date_issued.strftime("%Y-%m-%d"), p.issue.edition, p.sequence]),
  ...
  def to_representation(self, instance):
    return {
      'issue': {
        ...
        'url': settings.BASE_URL + reverse('api_issue', args=[instance.issue.title.lccn, instance.issue.date_issued.strftime("%Y-%m-%d"), instance.issue.edition]),
      },
...
class TitleSerializer(serializers.BaseSerializer):
  ...
  def to_representation(self, instance):
    return {
      ...
      'issues': [{
          'url': settings.BASE_URL + reverse('api_issue', args=[i.title.lccn, i.date_issued.strftime("%Y-%m-%d"), i.edition]),
